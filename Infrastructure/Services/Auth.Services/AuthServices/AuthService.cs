namespace Infrastructure
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<Response<LoginResponse>> LoginAsync(LoginRequest request)
        {
            ...
            return new Response<LoginResponse>(HttpStatusCode.OK, "Вход успешно выполнен", response);
        }
        ...
        public async Task<Response<RegisterResponse>> RegisterAsync(RegisterRequest request)
        {
            ...
            var role = await _dbContext.Roles.FirstOrDefaultAsync(x => x.RoleName == DefaultRoles.User);
            if (role != null)
            {
